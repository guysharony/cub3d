...
/*   Updated: 2020/01/25 17:51:51 by guysharon        ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		checkwallsvup(t_game *game)
{
	...
	while (game->map->map[a] != NULL)
	{
		...
		{
			b = ft_strlen(game->map->map[a - 1]);
			if (!checkchars(game->map->map[a], b, '1'))
				e += ft_error_line("Not surrounded by walls", "MAP", a);
		}
		a++;
	}
	if (e > 0)
